...
// Co-hosting properties are tagged "(CC)" in their Guesty listing name.
const isCoHosting = (name: string) => /\(CC\)/i.test(name);
...
export default function CoHostingPage() {
  ...
  // Restrict to co-hosting properties up front; everything below treats this as the universe.
  const ccUniverse = useMemo(() => {
    if (!data) return null;
    const ccProperties = data.properties.filter((p) => isCoHosting(p.name));
    const ccIds = new Set(ccProperties.map((p) => p.id));
    const ccReservations = data.reservations.filter((r) => ccIds.has(r.propertyId));
    return { properties: ccProperties, reservations: ccReservations };
  }, [data]);

  const filtered = useMemo(() => {
    if (!ccUniverse) return null;
    const reservations =
      propertyId === "all"
        ? ccUniverse.reservations
        : ccUniverse.reservations.filter((r) => r.propertyId === propertyId);
    const properties =
      propertyId === "all"
        ? ccUniverse.properties
        : ccUniverse.properties.filter((p) => p.id === propertyId);
    return { reservations, properties };
  }, [ccUniverse, propertyId]);
    ...
    <main className="mx-auto max-w-7xl space-y-10 px-6 py-10 animate-fade-in">
      ...
      <section className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-accent">
            Co-Hosting Portfolio Revenue
          </p>
          <h1 className="mt-2 text-display-lg tracking-tight text-text">
            {nyWeekdayDateLabel()}
          </h1>
          <p className="mt-1.5 text-sm text-muted">
            Live Guesty data — co-hosted properties only ({ccUniverse.properties.length} listings) · America/New_York · auto-refresh 15 min · last sync{" "}
        ...
        <div className="flex items-center gap-2">
          ...
          <PropertyFilter
            properties={ccUniverse.properties}
            value={propertyId}
      ...
      <section>
        ...
        <div className="mt-4 grid grid-cols-2 gap-4 md:grid-cols-4">
          <MetricCard label="Properties Available" value={today.totalProperties.toString()} sub="co-hosted listings" />
